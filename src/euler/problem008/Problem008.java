...
public class Problem008 implements Problem {
    ...
    public long solve() {
        ...
        return calculateProductOfNumbers(maximum);
    }

    private static int findMaximum() {
        final Pattern pattern = Pattern.compile("(9[123456789]{4})");
        final Matcher matcher = pattern.matcher(NUMBER);

        int maximum = 0;

        while (matcher.find()) {
            final int candidate = Integer.valueOf(matcher.group());

            maximum = maximum > candidate ? maximum : candidate;
        }

        return maximum;
    }

    private static int calculateProductOfNumbers(final int number) {
        int product = 1;

        final char[] digits = String.valueOf(number).toCharArray();

        for (final char digit : digits) {
            final int current = Character.getNumericValue(digit);
            product *= current;
        }
